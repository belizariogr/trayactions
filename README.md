...
- Keeps the tray icon alive across panel/host restarts: re-registers when the
  StatusNotifierWatcher appears, when the tray host re-registers, and via a
  periodic health check if the item is dropped while the process is still
  running. On COSMIC it waits for the panel Status Area (never D-Bus-activates
  the headless watcher, which would hide all tray icons).
